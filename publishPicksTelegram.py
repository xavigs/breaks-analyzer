...
async def send(message):
    await bot.send_message(chat_id=channelID, text=message, parse_mode=telegram.constants.ParseMode.MARKDOWN_V2)
...
picks = soup.select('div[class=card-body]')
today = datetime.now()
numPicks = 0
...
for pick in picks:
    ...
    if pickDate > today or numPicks < 1:
        ...
        if foundPick is None:
            message = '{} *{}*\n\n🏆 {} {}\n🎾 {}\n⏰ {}\n💰 @{}'.format(numPickEmoji, re.escape(pickDB['pick']), pickDB['competition'], flag, pickDB['event'], re.escape(pickDB['date']), pickDB['odd'])
            print(message)
